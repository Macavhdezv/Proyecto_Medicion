...
Nos aseguraremos que la variable comuna esté "match" con chilemapas

```{r}
df_final <- df_final |>
  mutate(comuna = str_to_upper(stringi::stri_trans_general(comuna, "latin-ascii")))
```

De esta forma se colocan los nombres en mayúsculas y sin tildes para que el join con el mapa no falle

```{r}
head(df_final$comuna)
```

# Preparar mapa desde chilemapas
...
colnames(mapa_comunas)
colnames(df_final)

mapa_sf <- mapa_comunas |> 
  mutate(nombre_comuna = str_to_upper(stringi::stri_trans_general(nombre, "latin-ascii"))) |>
